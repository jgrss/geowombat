...
.. list-table::
   ...
   * - ``YOLODetector``
     - Ultralytics YOLO
     - Axis-aligned + oriented boxes. License: AGPL-3.0.
   * - ``TorchGeoDetector``
...
polygon geometries with either axis-aligned envelopes or minimum
rotated rectangles. The result is a normal ``GeoDataFrame``:
...
``src.gw.to_yolo_dataset`` do it internally for axis-aligned and OBB
training, respectively (via the ``oriented=True`` flag).
