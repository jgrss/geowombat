...
authors:
  ...
  - name: Leonardo Hardtke
    affiliation: 3
  - name: Robert Denham
    orcid: 0000-0002-3342-7970
    affiliation: 3
  - name: Sharon Xu
    affiliation: 4
affiliations:
  - name: Independent Researcher, Brisbane, Australia
    index: 1
  - name: Department of Geography & Environment, The George Washington University, Washington, DC, USA
    index: 2
    ror: 00y4zzh67
  - name: The University of Queensland, School of the Environment, Brisbane, Australia
    index: 3
    ror: 00rqy9422
...
Several Python packages address parts of the geospatial raster workflow. Rioxarray [@rioxarray] adds rasterio-backed I/O and CRS-aware operations to xarray but no remote sensing-specific functionality such as vegetation indices, QA masking, or classification. Google Earth Engine [@gorelick2017google] processes global archives in the cloud but is proprietary, requires connectivity, and limits control over computation; Xee [@xee] bridges it to xarray but inherits those constraints. Open Data Cube [@odc] indexes analysis-ready data but requires infrastructure setup and ingestion, and Raster Vision [@rv] targets deep learning alone rather than the broader workflow.

Other contemporary packages overlap parts of GeoWombat's scope but remain in-memory and single-focus. scikit-eo [@tarazona2024scikiteo] supplies ready-made analysis algorithms: classification with calibration and accuracy assessment, spectral mixture analysis, image fusion, atmospheric correction, and a U-Net segmenter, but it offers neither cloud data access nor out-of-core scaling. EarthPy [@wasser2019earthpy] provides education-oriented utilities for stacking, masking, and plotting raster and vector data, omitting radiometry, classification, and large-dataset processing. rastereasy [@corpetti2025rastereasy] wraps I/O, resampling, reprojection, filtering, and light machine learning behind a NumPy-like Geoimage object, but loads whole images into memory.
...
All raster operations return Dask-backed xarray DataArrays. Computation is deferred until the user explicitly calls `.compute()` or `.gw.save()`, allowing GeoWombat to build optimized task graphs over arbitrarily large datasets. Chunk sizes are configurable and default to 512 x 512 pixels.
...
- **Object detection** (`detect/`): Tiled, georeferenced bounding-box detection that mirrors the `fit()`/`predict()`/`fit_predict()` shape of the machine learning module. It wraps Ultralytics YOLO [@ultralytics_yolo] and TorchGeo/torchvision detectors (Faster R-CNN, RetinaNet) behind a single `src.gw.detect()` accessor, and includes training-dataset construction, accuracy metrics, and optional Segment Anything Model (SAM) [@kirillov2023segment] refinement of detected boxes into polygons.
...
![Multi-band land cover classification using GeoWombat's scikit-learn pipeline integration. The `fit_predict` method demonstrates the library's ability to train and predict ML pipelines with minimal code.\label{fig:classification}](fit_predict.png){ width=90% }

# Research impact

GeoWombat has been used in peer-reviewed research spanning land cover mapping, agricultural monitoring, and environmental change detection [@michael_mann_2023_8215141; @10.1371/journal.pone.0254723;@agronomy14112575;@dorman2025; @Hersh03042021; @huang2025dust]. It is actively used in graduate courses at The George Washington University for teaching remote sensing and geospatial machine learning through [pygis.io](https://pygis.io) [@Mann2025]. The library's documentation includes tutorials covering all major features, and it is installable via pip and conda-forge, with over 578,000 downloads as of July 2026. The project maintains continuous integration testing across Python 3.10--3.13 on Linux, and welcomes contributions via its GitHub repository at [https://github.com/jgrss/geowombat](https://github.com/jgrss/geowombat).
