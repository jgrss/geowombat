...
class MapProcesses(object):
    ...
    def moving(
        ...
        Args:
            ...
            stat (Optional[str]): The statistic to compute.
                Choices are ['mean', 'std', 'var', 'min', 'max', 'perc', 'fourier_transform'].
        ...
        """
        # Support moving Fourier transform via existing moving_fourier
        if stat == "fourier_transform":
            ds = MapProcesses.moving_fourier(
                data, w=w, nodata=nodata, weights=weights
            )
            return ds["fourier_mean"]
    ...
    def moving_fourier(
        ...
        """
        Applies a moving window Fourier Transform and returns mean and variance.
        ...
        def fourier_transform(ch_bd):
            ...
            magnitude_spectrum = 20.0 * np.log(
        ...
        def apply_fourier(block):
            mean, stddev = fourier_transform(block)
            return mean, stddev

        # Apply the moving Fourier transform
        mean_result = generic_filter(
            data.values,
            lambda block: apply_fourier(block)[0],
            size=w,
            mode="constant",
            cval=nodata,
        )
        var_result = generic_filter(
            data.values,
            lambda block: apply_fourier(block)[1] ** 2,
            size=w,
            mode="constant",
            cval=nodata,
        )

        # Wrap results in xarray.Dataset
        ...
        var_da = xr.DataArray(
            ...
            dims=data.dims,
            coords=data.coords,
            name="fourier_variance",
        )

        return xr.Dataset(
            {"fourier_mean": mean_da, "fourier_variance": var_da}
        )
